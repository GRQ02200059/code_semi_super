...
def main():
    ...
    # 配置文件路径
    config_path = "configs/semi_supervised/ems_swin_semi_improved.py"
    ...
    # 创建模型模块
    log.info("创建Swin Transformer半监督学习模块...")
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="swin-best-{epoch:02d}-{val_iou:.4f}",
        monitor="val_iou",
        mode="max",
        save_top_k=3,
        save_last=True,
    ...
    # 开始训练
    log.info("开始Swin Transformer半监督学习训练...")
    trainer.fit(module, datamodule=datamodule)
    
    # 测试最佳模型
    log.info("测试Swin Transformer最佳模型...")
    trainer.test(module, datamodule=datamodule, ckpt_path="best")
    
    log.info("Swin Transformer半监督学习训练完成！")
